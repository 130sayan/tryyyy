...
# min_df=5,max_df=0.8,
docid2=1
def writeroc(listtrain,Ytrain,listtest,Ytest,fw2):
    pipe3=pipeline.Pipeline([('tfidf_vectorizer',TfidfVectorizer(tokenizer=tokenize,lowercase=True)),
    ('rochhio_classifier',NearestCentroid()) ])
    pipe3.fit(listtrain,Ytrain)
    roc=pipe3.predict(listtest)
    stri=str(f1_score(roc,Ytest,average='macro'))
    fw2.write(stri)
...
fw2=open('Results 2.txt',"w+",encoding="utf-8")
fw2.write("NumFeature"+" 0"+"\n")
fw2.write("ROCCHIO ")
writeroc(listtrain,Ytrain,listtest,Ytest,fw2)
fw2.write("\n")
fw2.close()
